...
Install the MCP extra and start the server using the existing Garmin token store.

    pip install -e ".[mcp]"
    python3 mcp_server.py

The default stdio transport is for local MCP clients. For ChatGPT, run the HTTP transport and expose it through a trusted HTTPS MCP tunnel or gateway. The streamable HTTP endpoint is /mcp.

    MCP_HOST=127.0.0.1 MCP_PORT=8000 python3 mcp_server.py --transport streamable-http

The server exposes read-only health, activity, profile, and device tools. It uses the token store at ~/.garminconnect by default; set GARMINTOKENS to use another directory. If no valid cached token exists, set GARMIN_EMAIL and GARMIN_PASSWORD in the server environment.
